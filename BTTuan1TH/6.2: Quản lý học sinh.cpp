...
class HocSinh {
	...
	public:
		...
		void xepLoai();
		void dtbCaoNhat(HocSinh hs[]);
		void timKiemTheoTen(HocSinh hs[]);
		void toanThapNhat(HocSinh hs[]);
...
void HocSinh::dtbCaoNhat(HocSinh hs[]) {
	HocSinh max = hs[0];
	for (int i = 1; i < 3; i++) {
		if (hs[i].dtb > max.dtb) {
			max = hs[i];
...
void HocSinh::timKiemTheoTen(HocSinh hs[]) {
    string ten;
    ...
    if (!found) {
        cout << "Khong tim thay hoc sinh co ten chua ' n" << ten << "'" << endl;
    }
}

void HocSinh::toanThapNhat(HocSinh hs[]) {
	HocSinh min = hs[0];
	for (int i = 1; i < 3; i++) {
...
int main() {
	...
	hs[0].dtbCaoNhat(hs);
	hs[0].timKiemTheoTen(hs);
	hs[0].toanThapNhat(hs);
	fclose(stdin);
	return 0;
}
